...
deploy-api:
	docker build --no-cache --platform linux/amd64 -f Dockerfile.backend -t $(REGISTRY)/$(PROJECT)/stroke-api:latest .
	docker push $(REGISTRY)/$(PROJECT)/stroke-api:latest
	oc rollout restart deployment/stroke-api

deploy-fe:
	docker build --no-cache --platform linux/amd64 -f Dockerfile.frontend -t $(REGISTRY)/$(PROJECT)/stroke-fe:latest .
